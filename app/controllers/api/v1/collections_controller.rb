...
class Api::V1::CollectionsController < ApplicationController
  ...
  def index
    ...
    render json: {collection: "goes here"}
  end
  ...
  def create
    ...
    #remember JSON is string, need to make into integers for max_players, times, year published, etc. 
    # game = Game.new(game_id: params[:collection][:game_id])
    # game.game_id = params[:collection][:game_id]
    # game.name = params[:collection][:name]
    # game.year_published = params[:collection][:year_published].to_i
    # game.min_players = params[:collection][:min_players].to_i
    ...
    render json: {status: "new collection successful", gameTest: game, user: user}
  end
  ...
  def collections_params
    ...
    params.require(:collection).permit(:user_id, :game_id, :name, :year_published, :min_players, :max_players, :description, :image_url, :min_playtime, :max_playtime)
  end 
end
